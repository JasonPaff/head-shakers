---
name: code-reviewer
description: Use this agent when you need comprehensive code review and quality analysis. Examples: <example>Context: User has just implemented a new authentication feature and wants it reviewed before merging. user: 'I just finished implementing the new OAuth integration with Clerk. Can you review the code?' assistant: 'I'll use the code-reviewer agent to conduct a thorough review of your OAuth integration implementation.' <commentary>Since the user is requesting code review of recently written code, use the code-reviewer agent to analyze the implementation for security, best practices, and integration patterns.</commentary></example> <example>Context: User has completed a database migration script and needs it validated. user: 'Here's the migration script for the new bobblehead collections schema. Please check it over.' assistant: 'Let me use the code-reviewer agent to review your database migration script for correctness and safety.' <commentary>Database migrations are critical and need thorough review for data integrity, performance, and rollback safety - perfect for the code-reviewer agent.</commentary></example> <example>Context: User has refactored a complex component and wants quality assessment. user: 'I refactored the CollectionGrid component to improve performance. Can you take a look?' assistant: 'I'll have the code-reviewer agent analyze your CollectionGrid refactoring for performance improvements and code quality.' <commentary>Component refactoring requires review for maintainability, performance impact, and adherence to React best practices.</commentary></example>
model: sonnet
---
