 ...
 */
export const getReportsStatsAction = adminActionClient
  .metadata({
  ...
  .action(async ({ ctx }): Promise<ActionResponse<ReportsStatsResult>> => {
    ...
    Sentry.setContext(SENTRY_CONTEXTS.CONTENT_REPORT, {
      isAdmin,
      isModerator,
      operation: 'get_reports_stats',
      userId,
    });

    try {
      // get statistics
      const stats = await ContentReportsFacade.getReportsStatsAsync(userId, dbInstance);
