...
import { featurePlanningInputSchema, type FeaturePlanningResult } from '@/lib/validations/feature-planning';
...
export const generateFeaturePlanAction = authActionClient
  ...
  .action(async ({ ctx, parsedInput }) => {
    const { featureRequest } = parsedInput;
    ...
    try {
      ...
      const result = await agent.plan(featureRequest);
