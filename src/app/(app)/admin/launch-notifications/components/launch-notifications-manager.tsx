...
import {
  getLaunchNotificationStatsAction,
  sendLaunchNotificationsAction,
} from '@/lib/actions/launch-notifications/admin.actions';
...
export function LaunchNotificationsManager({
  ...
  const [isConfirmDialogOpen, toggleConfirmDialog] = useToggle();
  ...
  const handleSendNotifications = useCallback(() => {
    ...
    toggleConfirmDialog.on();
  }, [stats.unnotifiedCount, toggleConfirmDialog]);

  const handleConfirmSend = useCallback(async () => {
    await sendNotificationsAsync();
    toggleConfirmDialog.off();
  }, [sendNotificationsAsync, toggleConfirmDialog]);
  ...
  return (
    <Fragment>
      <div className={'space-y-6'}>
        ...
        <Card>
          ...
          <CardContent>
            ...
            >
              <Conditional
                fallback={`Send to ${stats.unnotifiedCount} Subscribers`}
                isCondition={isSendingNotifications}
              >
                Sending...
              </Conditional>
            </Button>
            <Conditional isCondition={stats.unnotifiedCount === 0}>
        ...
        <Card>
          ...
          <CardContent>
            <Conditional
                          ...
                          <td className={'py-3'}>
                            <Conditional
                              fallback={
                                <span
                                  className={
                                    'inline-flex items-center gap-1 rounded-full bg-yellow-100 px-3 py-1 text-xs font-medium text-yellow-800'
                                  }
                                >
              ...
              isCondition={initialSignups.length === 0}
            >
      ...
      {/* Confirm Send Dialog */}
      <AlertDialog onOpenChange={toggleConfirmDialog.update} open={isConfirmDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Send Launch Notifications?</AlertDialogTitle>
            <AlertDialogDescription>
              This will send launch notification emails to {stats.unnotifiedCount} unnotified subscribers.
              This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={toggleConfirmDialog.off}>Cancel</AlertDialogCancel>
            <Button
              asChild
              onClick={() => {
            ...
            >
              <AlertDialogAction>Send Notifications</AlertDialogAction>
            </Button>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
