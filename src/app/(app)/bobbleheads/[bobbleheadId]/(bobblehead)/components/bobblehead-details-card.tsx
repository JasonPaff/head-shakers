...
export const BobbleheadDetailsCard = ({ bobblehead }: BobbleheadDetailsCardProps) => {
  ...
  const hasBasicInfo = bobblehead.characterName || bobblehead.series || bobblehead.manufacturer || 
    bobblehead.year || bobblehead.category || bobblehead.description;
    ...
    <Card>
      ...
      <CardContent className={"space-y-4"}>
        <DetailItem
          icon={UserIcon}
          label={"Character"}
          value={bobblehead.characterName}
        />
        <DetailItem
          icon={HashIcon}
          label={"Series"}
          value={bobblehead.series}
        />
        <DetailItem
          icon={TagIcon}
          label={"Manufacturer"}
        ...
        <DetailItem
          icon={FolderIcon}
          label={"Category"}
          value={bobblehead.category}
        />
        <DetailItem
          label={"Condition"}
          value={bobblehead.currentCondition}
          variant={"badge"}
        />
        {bobblehead.description && (
          <div>
            <span className={"text-sm text-muted-foreground"}>Description</span>
            <p className={"mt-1 leading-relaxed font-medium text-pretty"}>
              {bobblehead.description}
            </p>
          </div>
        )}
        {bobblehead.tags && bobblehead.tags.length > 0 && (
          <div>
